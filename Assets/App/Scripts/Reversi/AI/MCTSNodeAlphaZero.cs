...
namespace App.Reversi.AI
{
	...
	/// </summary>
	public class MCTSNode_AlphaZero
	{
		// PUCT計算用の探索定数 (AlphaZero論文の値)
		private const double C_PUCT = 4.0;

		public GameState State { get; }
		public MCTSNode_AlphaZero Parent { get; }
		public Dictionary<GameAction, MCTSNode_AlphaZero> Children { get; }
		...
		/// ノードのコンストラクタ
		/// </summary>
		public MCTSNode_AlphaZero(GameState state, MCTSNode_AlphaZero parent = null)
		{
			State = state;
			Parent = parent;
			Children = new Dictionary<GameAction, MCTSNode_AlphaZero>();
			VisitCount = 0;
			_winScore = 0.0;
			_value = 0.0f;
			_isEvaluated = false;
		...
		private double GetPUCTScore(GameAction action)
		{
			...
			// Children辞書はスレッドセーフではないため、lock内でアクセス
			if (Children.TryGetValue(action, out MCTSNode_AlphaZero child))
			{
				childVisitCount = child.VisitCount;
				// 子ノードの平均勝率（Valueは親視点に反転されているため、-をかけて元に戻す）
				qValue = (childVisitCount > 0)
					? (child._winScore / childVisitCount)
					: 0.0;
			}
			else
			{
			...
			// Q値は「相手視点」の勝率になっているため、-QValueで「自分視点」に戻す
		...
		/// </summary>
		public MCTSNode_AlphaZero Expand(GameAction action)
		{
			lock (_lock)
			{
				if (Children.TryGetValue(action, out MCTSNode_AlphaZero child))
				...
				// 存在しなければ新しく作成
				// 「ディープコピー版」のExecuteActionを使用する
				GameState nextState = ReversiSimulator.ExecuteAction(State, action);
				MCTSNode_AlphaZero newChild = new MCTSNode_AlphaZero(nextState, this);
		...
		/// <summary>
		/// このノードがNNの評価待ちか（葉ノードであり、まだ評価されていないか）
		/// </summary>
		public bool IsLeafAndNotEvaluated()
		{
			lock (_lock)
			{
				...
				// （ゲーム終了ノードはコンストラクタで評価済み(_isEvaluated = true)になる）
		...
		private float GetPriorPolicyForAction(GameAction action)
		{
			try
			{
				// NNモデルの出力（Policy）の形状 (12x12x5 = 720) に合わせてインデックスを計算
				int posIndex = action.Position.Row * 12 + action.Position.Col;
				int typeIndex = (int)action.Type; // 0~4
				int index = (typeIndex * 144) + posIndex; // (144 = 12*12)

				if (index >= 0 && index < _policy.Length)
				{
					// ソフトマックス（Softmax）の出力は確率なので、そのまま返す
